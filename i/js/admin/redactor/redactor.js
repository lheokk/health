...
(function($){
	...
	var Redactor = function(element, options) 
	{
		...
		this.opts = $.extend({
			...
			imageGetJson: '/i/js/admin/redactor/plugins/test.php', // url (ex. /folder/images.json ) or false		
	...
	Redactor.prototype = {
		...
		{		
			...
			frameHtml += '<html><head><link media="all" type="text/css" href="/i/css/admin/redactor/css/' + this.opts.css + '" rel="stylesheet"></head>';
	    ...
	    showImage: function()
	    {
			if ($.browser.msie) this.markerIE();
	         ...
	         var handler = $.proxy(function()
	         {
	        	...
	        	if (this.opts.imageGetJson !== false)
	        	{
					$.getJSON(this.opts.imageGetJson, $.proxy(function(data) {
						  ...
						  $.each(data, $.proxy(function(key, val)
						  {
								...
								//$("select[name=gallery]").click($.proxy(this.imageSetThumb, this));
						  }, this));
				...
				$("#set_gallery").click($.proxy(this.imageSetGallery, this));
	         }, this);            
	            
	        this.modalInit(RLANG.image, '/admin/gallery/redactor/image', 570, handler, true);
		...
		imageSetGallery: function()
		{
			var gallery = $("#gallery_id").val();
			var html = "<gallery id='gallery_id'>gallery_id:"+gallery+"</gallery>";
			this._imageSet(html);
		},
		...
		insertLink: function()
		{
			...
			{
				link = $('#redactor_link_url_button').val();
				text = "<button id='go_button'></button>";
			}				
