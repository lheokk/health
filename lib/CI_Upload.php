...
class CI_Upload {
	...
	{
        if (isset($this->allowed_types[0]) && $this->allowed_types[0] == '*')
        {
            return true;
        }
	...
	function set_error($msg)
	{
		...
		if (is_array($msg))
		{
			foreach ($msg as $val)
			{
				$msg = isset($g_lang[$val]) ? $g_lang[$val] : $val;
				$this->error_msg[] = $msg;
			}
		}
		else
		{
			$msg = isset($g_lang[$msg]) ? $g_lang[$msg] : $msg;
			$this->error_msg[] = $msg;
		}
	}
	...
	function mimes_types($mime)
	{
		...
		{
            // Список MiMe-типов файлов. Используется в классе СI - Upload
            $mimes = array
                (
