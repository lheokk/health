<?php
    ...
    if($_FILES['upload'])
    {
        ...
        else
        {
            ...
            move_uploaded_file($_FILES['upload']['tmp_name'], BASEPATH . "upl/news/" . $name);
            $full_path = '/upl/news/' . $name;
            $message = "Файл загружен";
            
            $size = getimagesize(BASEPATH . 'upl/news/' . $name);

            if ($size[0] < 50 OR $size[1] < 50){
                unlink(BASEPATH . 'upl/news/' . $name);
